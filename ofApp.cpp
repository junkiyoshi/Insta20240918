...
void ofApp::update() {
	...
	for (float x = ofGetWindowWidth() * -0.5; x <= ofGetWindowWidth() * 0.5 - span; x += span) {

		for (float y = ofGetWindowHeight() * -0.5; y <= ofGetWindowHeight() * 0.5 - span; y += span) {
			...
			for (int i = this->mesh.getNumVertices() - 4; i < this->mesh.getNumVertices(); i++) {
				...
				auto noise_location = glm::normalize(this->mesh.getVertex(i)) * 0.35;
				auto noise_value = ofMap(ofNoise(noise_location.x, noise_location.y, glm::length(this->mesh.getVertex(i)) * 0.005, ofGetFrameNum() * 0.003), 0, 1, 0, 6);
				int int_noise_value = noise_value;
				float param = noise_value - int_noise_value;
				param = abs(param - 0.5);
				auto alpha = param < 0.5 ? ofMap(param, 0, 0.5, 255, 0) : 0;

				this->mesh.addColor(ofColor(0, 0, 128, alpha));
			}
